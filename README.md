# 🐈‍⬛ 3D キャットオセロ 🐈

伝統的な白黒の駒の代わりに猫の絵文字を使用した、クラシックなオセロ（リバーシ）ゲームの現代的な実装です。目を引く3Dビジュアルスタイルで表現されています。

## 📊 ゲーム紹介スライド

![スライド1：3Dキャットオセロ](https://app.devin.ai/attachments/acec93e7-9f9e-4688-bcd3-0b183546a8ef/file_home_ubuntu_074152.png)

![スライド2：ゲームの特徴](https://app.devin.ai/attachments/acec93e7-9f9e-4688-bcd3-0b183546a8ef/file_home_ubuntu_074201.png)

![スライド3：今すぐプレイ！](https://app.devin.ai/attachments/acec93e7-9f9e-4688-bcd3-0b183546a8ef/file_home_ubuntu_074209.png)

## 🎮 ゲームのスクリーンショット

![日本語版キャットオセロ](https://app.devin.ai/attachments/b5b663a4-32a1-4619-a518-55dd1a53ce7f/ninja03_github_io_130720.png)

## 🎮 今すぐプレイ

以下のURLからオンラインでプレイできます：
- [GitHub Pages](https://ninja03.github.io/devin2/)
- [Devin Apps デプロイメント](https://cat-emoji-game-ld61orr0.devinapps.com)

## 📋 ゲームルール

オセロ（リバーシとしても知られる）は8×8のボードでプレイする戦略ボードゲームです：

1. ゲームは中央に4つの駒から始まります：2つの黒猫（🐈‍⬛）と2つの白猫（🐈）
2. 黒が常に先手です
3. プレイヤーは交互にボード上に駒を置きます
4. 有効な手は、新しく置いた駒と同じ色の別の駒の間に、少なくとも1つの相手の駒を「挟む」必要があります
5. 「挟まれた」すべての駒は現在のプレイヤーの色に裏返されます
6. プレイヤーが有効な手を打てない場合、そのターンはスキップされます
7. どちらのプレイヤーも有効な手を打てなくなったらゲーム終了です
8. ボード上に最も多くの駒を持つプレイヤーが勝利します

## 🎯 特徴

- 🎨 影と遠近感を持つ美しい3Dビジュアルエフェクト
- 🐱 伝統的な白黒の駒の代わりに猫の絵文字（🐈‍⬛と🐈）を使用
- 🎲 オセロ/リバーシのルールを完全に実装
- 🔄 有効な手は緑の点で強調表示
- 📊 リアルタイムのスコア追跡
- 🏆 ゲーム終了の検出と勝者の発表
- 🔄 いつでも再開できる新しいゲームボタン
- 📱 様々な画面サイズに対応したレスポンシブデザイン
- ✨ スムーズなアニメーション効果（駒の反転、ゲーム開始/終了など）
- 🌐 完全日本語対応のユーザーインターフェース

## 🛠️ 技術スタック

- **React**: ユーザーインターフェースを構築するためのフロントエンドライブラリ
- **TypeScript**: 型安全なコードのため
- **Tailwind CSS**: スタイリングとレスポンシブデザインのため
- **Vite**: 高速なビルドツールと開発サーバー
- **shadcn/ui**: UIコンポーネントライブラリ
- **Lucide Icons**: 美しく一貫性のあるアイコンのため

## 🌐 日本語化について

このゲームは完全に日本語化されており、以下の要素が日本語で表示されます：

- ゲームタイトル「3D キャットオセロ」
- 「現在のプレイヤー」表示
- ゲーム結果メッセージ（「引き分けです！」「黒/白の勝ちです！」）
- 「新しいゲーム」ボタン

## 🚀 開発

### 前提条件

- Node.js（v18以上）
- npm、yarn、またはpnpm

### セットアップ

1. リポジトリをクローンします：
   ```bash
   git clone https://github.com/ninja03/devin2.git
   cd devin2
   ```

2. 依存関係をインストールします：
   ```bash
   npm install
   ```

3. 開発サーバーを起動します：
   ```bash
   npm run dev
   ```

4. ブラウザを開いて `http://localhost:5173` にアクセスします

### 本番用ビルド

```bash
npm run build
```

ビルドされたファイルは `dist` ディレクトリに格納されます。

### デプロイ

このゲームは以下の方法でデプロイされています：

- **GitHub Pages**: リポジトリの`gh-pages`ブランチを使用
- **Devin Apps**: Devinの`deploy_frontend`コマンドを使用
